...
function place(){var q=document.querySelector("[name=ec-qty]"),n,w;if(!q||!R||!S)return;n=q.closest("[class$=__qty]")||q.parentElement;S.qty=q;w=R.querySelector(".w");if(w&&w.nextElementSibling!=n)w.after(n);q.oninput=q.onchange=trim}
...
function mount(k){if(document.querySelector(".cb"))return;var num=find(/NUMURUS/,"textarea"),por=find(/skaits/,"select"),keep=S?S.c:{},list=items(k),box=document.createElement("div"),html;if(!num)return;html=k=="m"?'<p class=g>Gaļa</p>'+list.filter(function(x){return x.g=="g"}).map(card).join("")+'<p class=g>Veģetārie</p>'+list.filter(function(x){return x.g=="v"}).map(card).join(""):list.map(card).join("");box.className="cb";box.innerHTML='<p class=w>'+M.w+'</p><h2></h2><div class=r><b class=n></b><div class=bar><i></i></div></div><p class=x></p>'+html+'<p class=k>Kods <b data-code>—</b></p>';(por?por.w:num.w).after(box);num.w.classList.add("hz");S={items:list,c:keep,m:0,num:num.el,sel:por&&por.el,por:por&&por.w};R=box;box.onclick=function(e){var b=e.target.closest("button"),key;if(!b||!b.dataset.act)return;e.preventDefault();key=b.closest(".d").dataset.key;if(b.dataset.act=="p"){if(S.m&&sum()<S.m)S.c[key]=(S.c[key]||0)+1}else if(S.c[key])S.c[key]--;paint()};if(por)por.el.onchange=trim;place();trim()}
...
const A="https://script.google.com/macros/s/AKfycbwYf3V5MElQQOl8YsvXZEBHNwFOsd80EeWYZujXrRNPMt4d_9LDpazaR-wer9m0dWldxw/exec",q=s=>document.querySelector(s),Q=s=>[...document.querySelectorAll(s)],H=v=>v=="Privātmāja"||v.startsWith("Nav būtiski");
...
x.onclick=()=>m.style.display="none";
p.oninput=()=>p.value=p.value.replace(/\D/g,"").slice(0,3);
document.addEventListener("click",e=>{let a=e.target.closest("a");if(a&&(a.href||"").includes("#piegades-laiks")){e.preventDefault();m.style.display="flex"}},true);
g.onclick=async()=>{let O=o.value.trim(),P=p.value.replace(/\D/g,"");if(!O||P.length!=3){r.textContent="Ievadi pasūtījuma numuru un telefona pēdējos 3 ciparus.";return}r.textContent="Meklējam...";try{let d=await fetch(A+"?order="+encodeURIComponent(O)+"&phone="+P).then(x=>x.json());r.innerHTML=d.success?"Tavs plānotais piegādes laiks:<br><b style='font-size:30px'>"+d.eta+"</b>":d.message}catch(_){r.textContent="Neizdevās pārbaudīt."}};
...
function F(){
 ...
 let h=H(R.find(x=>x.checked)?.value||""),row=f.closest(".ec-form__row")||f.parentElement;
...
function D(){
 ...
 let h=H(t?.value||""),cr=c.closest(".ec-form_row")||c.parentElement,nr=n.closest(".ec-form_row")||n.parentElement,z=c.closest(".form-control");
...
const W=(t,s,b,z=0)=>{wt.textContent=t;wm.textContent=s;pb=b;mode=z;w.style.display="flex"};
...
document.addEventListener("click",e=>{
 let b=e.target.closest("button");if(!b||["g","x","we","wg"].includes(b.id))return;
